    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *foundListingTransactions, NSError *error) {
        if (!error) {
            ...
            for (PFObject *listingTx in foundListingTransactions) {
                ...
                currentTransaction.objectId = listingTx.objectId;
                ...
                NSLog(@"Successfully loaded %d %@ %@", loadedTransactions.count, currentTransaction.objectId, currentTransaction.address1);
                /*
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    //if ([[segue identifier] isEqualToString:@"TransactionSelected"]) {
        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
    ZoomRexTransaction *object;
    TxDocumentsTableViewController* docsController;
        if(indexPath.section == 0) {
            object = sellingTransactions[indexPath.row];
            NSLog(@"selling %@ %d selected.", [object address1], sellingTransactions.count);
    
    
            docsController = segue.destinationViewController;
        } else {
            object = listingTransactions[indexPath.row];
            NSLog(@"listing %@ %d selected.", [object address1], listingTransactions.count);
            
            docsController = segue.destinationViewController;
            
        }
    [docsController setParentItem:detailItem];
    [docsController setDetailItem:object];
    //}
