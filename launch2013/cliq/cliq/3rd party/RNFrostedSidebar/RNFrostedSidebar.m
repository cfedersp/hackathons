...
//    CGFloat topPadding = leftPadding;
  ...
  [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
    ...
    CGFloat myItemSize = [[self.itemSizes objectAtIndex:idx] floatValue];
    NSLog(@"myItemSize is %f", myItemSize);
    CGFloat leftPadding = (self.width - myItemSize)/2;
    CGFloat topPadding = 50;
    NSLog(@"self.heightPadding is %f", self.heightPadding);
    CGRect frame = CGRectMake(leftPadding, self.heightPadding + topPadding * 2, myItemSize, myItemSize);
    view.frame = frame;
    view.layer.cornerRadius = frame.size.width/2.f;
    self.heightPadding += myItemSize+topPadding;
  }];
...
//    self.contentView.contentSize = CGSizeMake(0, items * (self.itemSize.height + leftPadding) + leftPadding);
  self.contentView.contentSize = CGSizeMake(0, self.heightPadding + 200);
}
