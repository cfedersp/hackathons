...
- (void)viewDidLoad {
    ...
    _theBayTekRequestProtocol = [[DQBayTekRequestProtocol alloc] initWithMachineID:(Byte)[_segmentControl selectedSegmentIndex] andDQuidIOSerialNumber:DQUIDIO_SERIAL_NUMBER andResponseDelegate:self];
    [_theBayTekRequestProtocol connect];
    [_segmentControl addTarget:self action:@selector(machineIDSelected:) forControlEvents:UIControlEventValueChanged];
...
-(void) machineIDSelected:(id)sender {
    ...
    _theMachineID = [_segmentControl selectedSegmentIndex];
...
-(void) onMachineIDReceived:(Byte) machineID {
    ...
    // Ugly hack for PrizeHub id
    [_segmentControl setSelectedSegmentIndex:machineID!=3?machineID:machineID-1];
    [self machineIDSelected:_segmentControl];
}
...
-(void) onMajGameVerReceived:(Byte) majVer {
    ...
    UITableViewCell* cell = [[TableCellsRegistry instance] getCellForCallbackSelName:NSStringFromSelector(_cmd)];
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSMutableDictionary *cellInfo = CELLS_REGISTRY[[indexPath row]];
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSMutableDictionary *cellInfo = CELLS_REGISTRY[[indexPath item]];
